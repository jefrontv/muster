...
describe('PtyHandler', () => {
  ...
  it("does not forward Muster's own NODE_ENV into the spawned shell", async () => {
  ...
  it('replaces an ambient TERM=dumb when no explicit TERM is supplied', async () => {
    ...
    expect(spawnEnv.env.TERM_PROGRAM).toBe('Muster')
  })
  ...
  it('revive restores pane identity env alongside hook-server coordinates', async () => {
    ...
    expect(callArgs.env.TERM_PROGRAM).toBe('Muster')
  })
