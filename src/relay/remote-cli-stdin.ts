export function shouldReadRemoteCliStdin(argv: string[]): boolean {
  ...
  // Why: `*-stdin` flags (`--text-stdin`, ...) declare a stdin payload directly
  // in the flag name; the full-CLI bridge (#7716) must forward stdin for them
  // the same way local shells provide it.
