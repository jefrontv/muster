...
import { getChatModeExperimentalSearchEntry } from './chat-mode-experimental-search-entry'
...
export const getExperimentalPaneSearchEntries = createLocalizedCatalog(
  (): SettingsSearchEntry[] => [
    ...
    getChatModeExperimentalSearchEntry(),
    {
      title: translate(
    ...
    },
    {
      ...
      description: translate(
        ...
        'Stops idle background agent terminals after the configured idle window and resumes supported sessions when opened again. Agent sleep preserves launch options for agents started by Muster; manually started agents may resume with current Muster defaults.'
      ),
      keywords: [
        ...translateSearchKeyword(
          'auto.components.settings.experimental.search.0d24759f14',
...
export function getExperimentalSearchEntry() {
  return {
    ...
    chatMode: findEntry(
      translate('auto.components.settings.experimental.search.chatMode.title', 'Chat mode')
    ),
