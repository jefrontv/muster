...
import { ChatModeExperimentalSetting } from './ChatModeExperimentalSetting'
...
export function ExperimentalPane({
  ...
  const showChatMode = matchesSettingsSearch(searchQuery, [getExperimentalSearchEntry().chatMode])
  ...
  return (
    <div className="space-y-4">
      ...
      {showChatMode ? (
        <ChatModeExperimentalSetting settings={settings} updateSettings={updateSettings} />
      ) : null}
            ...
            <div className="min-w-0 shrink space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                {translate(
                  ...
                  'Stops idle background agent terminals after the configured idle window and resumes supported sessions when you open them again. Agent sleep preserves launch options for agents started by Muster. Manually started agents may resume with your current Muster defaults. Experimental while we tune the safety model.'
                )}
              </p>
            </div>
            <SettingsSwitch
          ...
          {agentHibernationEnabled ? (
            <NumberField
              ...
              description={translate(
                ...
                'How many idle minutes a completed background agent must wait before Muster can sleep it.'
              )}
