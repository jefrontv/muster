  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Muster. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Muster. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Muster workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Muster. The project folder on disk will not be deleted.'
