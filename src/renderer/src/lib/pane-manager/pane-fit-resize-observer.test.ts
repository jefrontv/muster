...
describe('requestStablePaneFit stability window', () => {
  beforeEach(() => {
    mockResizeObservers = []
    nextRafId = 1
    pendingRafs = new Map()
  ...
  it('fits on the first repeated proposal by default', () => {
    const pane = createPane(() => ({ cols: 80, rows: 24 }))
  ...
  // The macOS Space-switch jitter: the grid held a wrong value long enough to satisfy a two-frame
  // settle, so the pane reflowed mid-transition and reflowed back once the metrics recovered.
  it('waits out a wobble that outlives the default settle when more proposals are required', () => {
    // Three frames of a transient grid, then the recovered one.
    const wobble = { cols: 70, rows: 24 }
    const recovered = { cols: 80, rows: 24 }
    let frame = 0
    const pane = createPane(() => (frame++ < 3 ? wobble : recovered))

    requestStablePaneFit(pane, undefined, { stableProposals: 6, maxFrames: 30 })

    // The default two-frame settle would have reflowed on the transient grid inside this window.
  ...
  it('still fits once the frame cap is reached on a grid that never settles', () => {
    let cols = 60
    const pane = createPane(() => ({ cols: cols++, rows: 24 }))

    requestStablePaneFit(pane, undefined, { stableProposals: 6, maxFrames: 8 })
