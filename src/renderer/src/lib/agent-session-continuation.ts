...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Muster pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Muster session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Muster status hints:', ...statusHints] : []),
    '',
