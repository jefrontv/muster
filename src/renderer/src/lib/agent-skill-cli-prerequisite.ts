...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Muster may show a system prompt to register the Muster CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Muster needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Muster CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Muster CLI in PATH.'
          )
    )
    return null
  }
  ...
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Muster CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Install the Muster CLI before running agent skill setup.'
          )
      }
    )
    return
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Muster CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'Muster could not check your Windows user PATH'
      ),
  ...
  if (status.pathConfigured === false) {
    ...
    toast.warning(
      translate(
        ...
        'Muster CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Muster CLI directory to PATH before setup.'
          )
      }
    )
  }
