...
import { createHarness, SELF_PID } from './agent-status-extension-harness.test-fixture'
...
describe('getPiAgentStatusExtensionSource', () => {
  ...
  it('keeps OMP runtime status payloads unchanged by Pi session metadata', async () => {
    ...
    expect(harness.fetchMock.mock.calls[0]?.[1]?.body).toBe(
      JSON.stringify({
        ...
        env: 'env-1',
        version: '1.2.3',
        payload: { hook_event_name: 'agent_start', subagents: [] }
      })
    )
  })
  ...
  it('tracks persistent OMP sessions and clears ephemeral session ids', async () => {
    ...
    expect(
      ...
      { hook_event_name: 'agent_start', session_id: 'omp-session-8', subagents: [] },
      {
        hook_event_name: 'before_agent_start',
        prompt: 'hi',
      ...
      { hook_event_name: 'agent_end', subagents: [] }
    ])
  })
