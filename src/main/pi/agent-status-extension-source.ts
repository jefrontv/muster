...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '// Muster receiver from building an unbounded queue of obsolete snapshots.',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('Muster hook delivery timed out'))",
    ...
    '    ])',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because Muster',
    '    // is unavailable or the loopback request failed (e.g. Muster restart).',
    ...
    '// reach Muster. curl.exe runs on the Windows side, where 127.0.0.1 IS the',
    '// listener Muster binds. Fire-and-forget: blocking on the spawn would stall',
