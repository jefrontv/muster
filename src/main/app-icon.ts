...
import watercolorMacDockIcon from '../../resources/app-icons/muster-watercolor.png?asset&asarUnpack'
import blueIcon from '../../resources/app-icons/muster-blue.png?asset'
...
const MAC_DOCK_ICON_SCRIPT = [
  ...
  'if image is missing value then error "Muster app icon image could not be loaded"',
  "set ok to current application's NSWorkspace's sharedWorkspace()'s setIcon:image forFile:appPath options:0",
  'if ok is false then error "Muster app icon could not be persisted"'
]
...
const MAC_DOCK_ICON_CLEAR_SCRIPT = [
  ...
  'if ok is false then error "Muster app icon could not be cleared"'
]
