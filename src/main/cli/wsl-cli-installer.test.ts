...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Muster\\resources\\bin\\orca.exe'
) {
  return {
    platform: 'win32',
    commandName: 'orca',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\Muster\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
set -euo pipefail
# Muster managed WSL CLI launcher
# ORCA_WIN_LAUNCHER_B64=QzpcUHJvZ3JhbSBGaWxlc1xPcmNhXHJlc291cmNlc1xiaW5cb3JjYS5jbWQ=
ORCA_WIN_LAUNCHER='C:\\Program Files\\Muster\\resources\\bin\\orca.cmd'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# Muster managed WSL CLI PowerShell bridge')
      ) {
      ...
      if (files.get(legacyCommandPath)?.includes('# Muster managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# Muster managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows Muster launcher', async () => {
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\Muster\\resources\\bin\\orca.exe',
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\Muster\\orca.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Muster\\orca.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\Muster\\orca.cmd',
      '/home/alice/.local/share/orca/orca-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'Muster WSL CLI requires Windows interop and could not find powershell.exe.'
    )
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\Muster\\bin\\orca.cmd',
      '/home/alice/.local/share/orca/orca-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Muster\\bin\\orca.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\Muster\\resources\\bin\\orca.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Muster\\resources\\bin\\orca.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\Muster\\resources\\bin\\orca.exe'
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\Muster\\resources\\bin\\orca.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# Muster managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\Muster\\resources\\bin\\orca.exe')
      },
      wslRunner: wsl.runner
    })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installer = new WslCliInstaller({
        ...
        hostInstaller: {
          getStatus: async () =>
            makeHostStatus('C:\\Program Files\\Muster\\resources\\bin\\orca.exe')
        },
        wslRunner: runner
      })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom Muster\\resources\\bin\\orca.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain(
      "ORCA_WIN_LAUNCHER='D:\\Custom Muster\\resources\\bin\\orca.exe'"
    )
  })
