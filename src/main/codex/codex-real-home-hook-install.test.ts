...
describe('ensureRealHomeCodexHookState (install)', () => {
  it('creates hooks.json with the Muster entry in every managed event for a fresh home', () => {
    grantSucceeds()
  ...
  it("keeps later user handler trust positions stable inside Muster's hook group", () => {
    grantSucceeds()
    ensureRealHomeCodexHookState({ hooksEnabled: true, userDataPath: userDataDir })
    const installed = readRealHooksJson()
...
describe('ensureRealHomeCodexHookState (opt-out sweep)', () => {
  it('rebases trust when a user appended hooks after Muster installed', () => {
    grantSucceeds()
  ...
  it('removes only Muster entries and reports the removed lane', () => {
    grantSucceeds()
    const userStop = {
      matcher: 'deploy-*',
  ...
  it('removes only hash-proven Muster trust from a mixed hook group', () => {
    const material = getCodexManagedHookInstallMaterial()
