...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    "// OpenCode's Node process (not Muster's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Muster restart',
    ...
    '      // writeEndpointFile on the Muster side) allocates a fresh inode on',
    ...
    '  // frozen when OpenCode was fork()ed — stale after a Muster restart. The',
    '  // file is rewritten on every Muster start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Muster',
    ...
    '// per turn through Muster (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root Muster pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer Muster since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-Muster-instance.',
    ...
    '    // because Muster is unavailable or the local loopback request failed.',
    '  }',
    '}',
    '',
