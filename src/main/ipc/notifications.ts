...
// Why: every source that has its own switch, so the gate is one lookup instead of one `||` per kind.
type NotificationSourceToggle = {
  [K in keyof NotificationSettings]: NotificationSettings[K] extends boolean ? K : never
}[keyof NotificationSettings]

const NOTIFICATION_TOGGLE_BY_SOURCE: Partial<
...
// renderer round trip; set on registration, so a dispatch before that is a no-op, not a crash.
let mainNotificationDispatch:
  | ((
...
export function registerNotificationHandlers(store: Store, runtime?: OrcaRuntimeService): void {
  ...
  // Why: one body, two entry points — the IPC handler below and dispatchMainProcessNotification.
  const dispatchNotificationRequest = (
    args: NotificationDispatchRequest
  ): NotificationDispatchResult | Promise<NotificationDispatchResult> => {
    ...
    const sourceToggle = NOTIFICATION_TOGGLE_BY_SOURCE[args.source]
    if (sourceToggle !== undefined && !settings[sourceToggle]) {
    ...
    if (runtime && args.source !== 'test') {
      const dedupeKey = args.dedupeKey ?? args.worktreeId ?? args.worktreeLabel ?? 'global'
    ...
    // Why: an ActiveCollab change carries no worktree, so focus alone decides — the window in
    // front of the user is the one that will show them the task.
    const focusSuppressible =
      args.isActiveWorktree === true || args.source.startsWith('activecollab-')
    if (
