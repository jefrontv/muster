...
describe('Store', () => {
  ...
  it('normalizes malformed visible task providers on load', async () => {
    ...
    const store = await createStore()
    expect(store.getSettings().visibleTaskProviders).toEqual(['gitlab', 'jira', 'activecollab'])
  })
  ...
  it('preserves a deliberate Jira provider opt-out after migration', async () => {
    ...
    const store = await createStore()
    // ActiveCollab is appended because this profile predates it; the Jira opt-out still holding is
    // the thing under test.
    expect(store.getSettings().visibleTaskProviders).toEqual(['gitlab', 'activecollab'])
  })

  it('preserves a deliberate ActiveCollab provider opt-out after migration', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
  ...
  it('flips a persisted chat-mode off default on once', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
      settings: { experimentalChatMode: false },
      ui: {},
      githubCache: { pr: {}, issue: {} },
      workspaceSession: {}
    })
    ...
    const store = await createStore()
    expect(store.getSettings().experimentalChatMode).toBe(true)
    expect(store.getSettings().experimentalChatModeDefaultedOnForMuster).toBe(true)
  })

  it('preserves a post-migration chat-mode opt-out', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
    ...
    const store = await createStore()
    expect(store.getSettings().experimentalChatMode).toBe(false)
    expect(store.getSettings().experimentalChatModeDefaultedOnForMuster).toBe(true)
  })
