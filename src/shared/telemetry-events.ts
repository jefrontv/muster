...
const onboardingFeatureSetupFeatureSchema = z.enum(['orchestration', 'linear_tickets'])
const onboardingFeatureSetupSelectionSchema = {
  linear_tickets: z.boolean(),
  orchestration: z.boolean(),
  selected_count: z.number().int().min(0).max(1)
} as const
...
function hasMatchingOnboardingFeatureSetupSelectedCount(
  ...
  const selectedCount = props.orchestration ? 1 : 0
