 ...
 * sessions. Folding here knowingly treats canonically equivalent names as one,
 * which is exact on APFS but permissive on byte-exact Linux/SSH hosts — an
 * acceptable trade, since only comparison keys are affected.
 */
...
export function relativePathInsideRoot(rootPath: string, candidatePath: string): string | null {
  ...
  return comparisonRoot.startsWith('//wsl/')
    ...
    : sliceCandidatePastRootSegments(comparisonRoot, normalizedCandidate)
}

/**
 * Why: skip whole root segments rather than a character count. Comparison
 * keys are NFC-folded while the returned suffix stays raw, and NFD spellings
 * are longer than their NFC keys, so a char-count slice lands mid-segment.
 */
