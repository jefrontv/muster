...
describe('command token scanner', () => {
  ...
  it('preserves quoted command paths with spaces', () => {
    expect(getFirstCommandToken('"C:\\Program Files\\Muster\\codex.cmd" --resume')).toBe(
      'C:\\Program Files\\Muster\\codex.cmd'
    )
  })

  it('extracts path basenames without allocating path segment arrays', () => {
    expect(getCommandTokenPathBasename('C:\\Program Files\\Muster\\codex.cmd')).toBe('codex.cmd')
