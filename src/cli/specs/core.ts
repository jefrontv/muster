...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['open'],
    summary: 'Launch Muster and wait for the runtime to be reachable',
  ...
  },
  {
    ...
    summary: 'Start Claude Code Agent Teams in the current Muster terminal',
    ...
    notes: [
      ...
      'Must be run from inside a Muster terminal. Starts Claude Code Agent Teams in the current pane and opens teammates as native Muster splits.'
    ],
    examples: ['orca claude-teams', 'orca claude-teams --resume <session-id>']
  },
  {
    path: ['repo', 'list'],
    summary: 'List repos registered in Muster',
    usage: 'orca repo list [--json]',
    allowedFlags: [...GLOBAL_FLAGS]
  },
  {
    path: ['repo', 'add'],
    summary: 'Add a project to Muster by filesystem path',
  ...
  {
    path: ['worktree', 'list'],
    summary: 'List Muster-managed worktrees',
  ...
  },
  {
    ...
    summary: 'Show the Muster-managed worktree for the current directory',
    usage: 'orca worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing Muster worktree without spelling out $PWD.'
    ],
    examples: ['orca worktree current', 'orca worktree current --json']
  },
  {
    path: ['worktree', 'create'],
    summary: 'Create a new Muster-managed worktree',
    usage:
    ...
    notes: [
      ...
      'By default, Muster records the new worktree as a child of the caller context when it can infer one from the Muster terminal or current directory.',
      'If --repo is omitted, Muster infers the repo from the current Muster-managed worktree.',
      ...
      '--no-parent only affects Muster lineage; omit --base-branch to use the repo default base, or pass the default base ref explicitly for independent top-level work.',
      'By default this creates the worktree and its first terminal without switching the active Muster view.',
    ...
    examples: [
      ...
      'orca worktree create --project github:jefrontv/muster --host runtime:gpu --name benchmark --json',
  ...
  {
    path: ['worktree', 'set'],
    summary: 'Update Muster metadata for a worktree',
    usage:
  ...
  },
  {
    ...
    destructive: true,
    summary: 'Remove a worktree from Muster and git',
  ...
  {
    path: ['terminal', 'list'],
    summary: 'List live Muster-managed terminals',
