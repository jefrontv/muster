...
export function launchOrcaApp(): void {
  ...
  throw new RuntimeClientError(
    'runtime_open_failed',
    'Could not determine how to launch Muster. Start Muster manually and try again.'
  )
}
    ...
    const onClose = (code: number | null, signal: NodeJS.Signals | null): void => {
      ...
      finish(
        new RuntimeClientError(
          ...
          typeof code === 'number'
            ? `Muster serve exited before printing valid recipe JSON with code ${code}.`
            : `Muster serve exited before printing valid recipe JSON via ${signal}.`
        )
      )
    }
...
function resolveForegroundOrcaExecutable(): string {
  ...
  throw new RuntimeClientError(
    'runtime_serve_failed',
    'Could not determine how to start Muster server. Set ORCA_APP_EXECUTABLE to the Muster executable.'
  )
}
