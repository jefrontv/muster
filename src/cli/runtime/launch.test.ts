...
describe('serveOrcaApp', () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset()
    process.env.ORCA_APP_EXECUTABLE = '/Applications/Muster.app/Contents/MacOS/Muster'
  })
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      ...
      const appPath = join(root, 'Muster.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Muster')
  ...
  it('rejects when the server exits without valid recipe JSON', async () => {
    ...
    await expect(result).rejects.toMatchObject({
      ...
      message: 'Muster serve exited before printing valid recipe JSON with code 0.'
    })
...
describe('launchOrcaApp', () => {
  ...
  it('handles asynchronous detached spawn errors without throwing', async () => {
    process.env.ORCA_APP_EXECUTABLE = '/missing/Muster'
    const child = new FakeChildProcess()
    spawnMock.mockReturnValue(child)
