...
export const MAC_CRASH_REPORT_GLOB = '~/Library/Logs/DiagnosticReports/Muster-*.ips'
...
export function serveSignalExitError(
  ...
  if (!signal) {
    return new RuntimeClientError(
      ...
      'Muster serve exited without reporting an exit code or signal.'
    )
  }
  if (platform !== 'darwin' || signal !== 'SIGABRT') {
    return new RuntimeClientError('runtime_serve_failed', `Muster serve exited via ${signal}.`)
  }
  ...
  return new RuntimeClientError(
    'runtime_serve_failed',
    'Muster serve aborted with SIGABRT on macOS. This most often happens at application startup, when the process cannot register with the macOS window server, which is common in restricted or sandboxed environments, SSH sessions without a GUI login, and CI.',
    {
      nextSteps: [
