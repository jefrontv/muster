...
async function openFeatureTourFromMenu(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ BrowserWindow, Menu }) => {
    const featureTourItem = Menu.getApplicationMenu()
      ...
      ?.submenu?.items.find((item) => item.label === 'Explore Muster')

    if (!featureTourItem) {
      throw new Error('Explore Muster menu item was not registered')
    }
...
test.describe('Feature tour modal', () => {
  ...
  test('opens from the Help menu and renders the workflow rail', async ({
    ...
    await expect(orcaPage.getByRole('dialog', { name: 'Get to know Muster' })).toBeVisible({
      timeout: 10_000
    })
    await expect(orcaPage.getByText('Reopen any time from Help > Explore Muster.')).toBeVisible()
    ...
    await expect(
      orcaPage.getByText(
        "Run your app in Muster's browser, send selected UI elements to agents, and let your agents interact with your webpage."
      )
    ).toBeVisible()
    await expect(orcaPage.getByRole('heading', { name: 'Browser Use skill' })).toBeVisible()
    await expect(
      orcaPage.getByText("Enables agents to navigate and verify pages in Muster's browser.")
    ).toBeVisible()
    await expect(orcaPage.getByRole('heading', { name: 'CLI skill' })).toHaveCount(0)
    await expect(orcaPage.getByText('With the Muster CLI skill', { exact: false })).toHaveCount(0)
  })
