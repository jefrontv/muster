...
This profile investigated high Muster renderer memory while working in
...
- Muster runtime was reachable through the packaged CLI fallback. The public
...
- `orca tab list --worktree all --json` returned `tabs: []`. There were no live
  Muster browser tabs in the measured session.
...
2. The browser-pane retention fix is still useful: inactive worktree browser
   ...
   state remains in Muster, and automation-visible webviews stay mounted so
   agent-browser can keep driving them.
3. Resource Usage was using `app.getAppMetrics().memory.workingSetSize` for
   Muster app buckets. On macOS this can count large shared Electron/Chromium
...
- Browser panes now mount their backing webview only when the pane is active or
  ...
  without sleeping the main Muster renderer.
...
The current packaged Muster app was not running this worktree's patched code
...
conflicts. That meant Settings could not replace a Muster-owned stale launcher,
forcing profiling to use the packaged CLI fallback.

The follow-up fix teaches the installer to recognize only generated Muster Unix
...
run with `--json`, including host memory, Muster app process buckets, worktree
