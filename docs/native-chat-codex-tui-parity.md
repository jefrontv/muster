...
This note maps Codex TUI behavior to Muster native chat on branch
`inspect/pr-5824-native-chat`. It is intentionally concrete: the current Muster
...
## Current Muster Architecture

Muster native chat currently sends through the hosted terminal PTY. The composer
...
- Native chat discovers skills through Muster's skills IPC with the active
...
- Native chat has interactive cards sourced from Muster's existing agent status
  hooks. This is good for common question/approval flows, but it is not the full
  Codex approval overlay model.

Recommended route:

- Keep PTY fallback for anything not represented in Muster hooks.
