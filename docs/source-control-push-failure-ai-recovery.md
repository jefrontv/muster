...
- Switching worktrees or branches must not show another worktree or branch's
  push failure.
- External edits or another Muster window may make the failure stale. Do not add
...
- Residual risks: Classifier false positives remain possible for unusual local
  ...
  panels are renderer-local, so another Muster window or an external terminal can
