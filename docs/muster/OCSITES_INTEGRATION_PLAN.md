...
| **SSH host-key policy is weaker than ocsites** | **medium — open** | Orca accepts any host key and only records its SHA256 fingerprint (`src/main/ssh/ssh-connection.ts:1131`). ocsites pinned keys trust-on-first-use via a persisted `~/.ssh/known_hosts` (`deploy/ssh_util.py`), so a *changed* key on a known host was rejected. Site runs therefore lose MITM protection on key rotation while sending a stored SSH password. Deliberately **not** changed during the port: `hostVerifier` is global to every Orca SSH feature, so tightening it is an app-wide security decision, not a site-pipeline one. Tracked as follow-up: add a known-hosts store and TOFU pinning, at minimum for connections that carry a stored password. |
...
- Native helper bundle names (`Orca Computer Use.app`, `orca-notification-status`) are unchanged. They are internal helper binaries built from Swift sources, not user-facing identity; renaming them means editing the SwiftPM package plus two build scripts for no behavioural gain.
