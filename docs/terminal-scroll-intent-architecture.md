...
an escalation point for Muster because private internals increase xterm upgrade
risk.

## Current Muster Risk Points

The current Muster branch already has several independent scroll actors:
...
The design must preserve Muster's terminal performance priorities:
...
- Whether Muster should patch private xterm bottom-scroll internals. The reference
  pattern does this for strict ownership, but Muster should first try public API
