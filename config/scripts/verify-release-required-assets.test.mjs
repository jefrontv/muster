  ...
  it('includes both mac updater ZIP names for the tag version', () => {
    expect(getRequiredReleaseAssetNames('v1.4.27')).toEqual(
      expect.arrayContaining([
        'Muster-1.4.27-mac.zip',
        'Muster-1.4.27-mac.zip.blockmap',
        'Muster-1.4.27-arm64-mac.zip',
        'Muster-1.4.27-arm64-mac.zip.blockmap'
      ])
    )
  })
  ...
  it('extracts relative and absolute manifest asset names', () => {
    expect(
      extractManifestAssetNames(
        [
          'files:',
          '  - url: Muster-1.4.27-arm64-mac.zip',
          '  - url: https://example.com/downloads/orca-windows-setup.exe',
          'path: orca-linux.AppImage'
        ].join('\n')
      )
    ).toEqual(['Muster-1.4.27-arm64-mac.zip', 'orca-windows-setup.exe', 'orca-linux.AppImage'])
  })
})
...
describe('verifyRequiredReleaseAssets', () => {
  it('fails when a manifest-referenced asset has not been uploaded', async () => {
    ...
    const assets = required.filter((name) => name !== 'Muster-1.4.27-arm64-mac.zip')
      ...
      .mockResolvedValueOnce(
        jsonResponse(
          [
            ...
            '  - url: Muster-1.4.27-arm64-mac.zip',
            '    sha512: test',
            'path: Muster-1.4.27-arm64-mac.zip'
          ].join('\n')
        )
      )
    ...
    ).rejects.toThrow('Missing: Muster-1.4.27-arm64-mac.zip')
