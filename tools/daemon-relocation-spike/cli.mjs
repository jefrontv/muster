...
Required (launch mode):
  --app-dir <path>    Path to a packaged win-unpacked build (contains Muster.exe)
...
Options:
  --tier <t>          File-set tier to copy: "full" (default), "no-gpu", "minimal"
                        full    = Muster.exe + icudtl.dat + both snapshot blobs +
                        ...
                        no-gpu  = full minus GPU/render DLLs (libEGL, libGLESv2,
                                  ...
                                  ffmpeg.dll kept
                        minimal = Muster.exe + icudtl.dat + both snapshots +
