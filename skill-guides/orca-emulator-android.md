...
description: >
  Control an Android emulator / device from inside Muster using the `orca` CLI.
...
# Muster Emulator — Android (adb / emulator powered)

Drive an Android emulator or adb-connected device **from within Muster** using
...
## Prerequisites (surfaced by Muster)

- **Android Studio / Android SDK** installed, with `ANDROID_HOME` (or
  `ANDROID_SDK_ROOT`) set. Muster also checks the per-OS default location
...
Muster returns a clear message when the SDK is missing
...
Muster owns backend routing and the per-worktree active-device registry. The
Android backend converts Muster's normalized 0–1 coordinates to device pixels and
...
(top-left origin) — never pixels; Muster converts using the live screen size.
...
- **All coordinates are normalized 0..1** (top-left origin), never pixels — Muster
