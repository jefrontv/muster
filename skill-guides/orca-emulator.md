...
name: orca-emulator
description: >
  Control a mobile (iOS) emulator / simulator stream from inside Muster using the `orca` CLI.
  Use for taps, gestures, typing, hardware buttons, camera injection, permissions, accessibility tree, and more — all while seeing the live view in Muster's emulator pane.
  Prefer this over raw `npx serve-sim` or direct simctl when running agents inside Muster (the orca surface handles device scoping, helper lifecycle, and worktree context).
  Complements the orca-cli skill for terminals, worktrees, and the built-in browser.
license: Apache-2.0
---

# Muster Emulator (serve-sim powered)

Drive an Apple Simulator (iOS / iPad / Watch) **from within Muster** using `ORCA emulator ...` commands (or `ORCA emulator exec` for raw power). This wraps the excellent [serve-sim](https://github.com/EvanBacon/serve-sim) open-source tool so agents get a consistent Muster-native CLI surface, automatic helper management, and seamless integration with Muster's live emulator pane (the visual "preview" surface).

The underlying serve-sim helper captures the real simulator framebuffer (via private SimulatorKit / IOSurface for low-latency 60fps H.264 or MJPEG) and exposes a WebSocket control channel. Muster's bridge owns the helper processes and per-worktree "active emulator" state so unqualified commands "just work" on whatever device/pane is current for the worktree.
...
- The user/agent wants to **tap, swipe, drag, pinch, or press hardware buttons** on a running iOS simulator while seeing the live result in Muster.
...
- You are inside a Muster worktree/terminal and want the emulator to be **workspace-scoped** (like browser tabs) with explicit targeting when needed.
- The agent should use Muster's preview pane instead of external Simulator.app or raw serve-sim URLs.
...
## Prerequisites (enforced / surfaced by Muster)

- macOS host (with Xcode Command Line Tools: `xcrun --version`).
- A booted simulator (`xcrun simctl list devices booted` or let Muster/attach help boot one).
- Node available (for the serve-sim bits; Muster bundles the CLI surface).
- macOS 14+ recommended for full camera injection features.

Muster will give clear errors if these are missing (e.g. "emulator commands require macOS + Xcode tools").
...
│ Muster EmulatorBridge│ ───────────────► │ serve-sim-bin   │ ────────────► │ iOS Simulator│
...
Agents use the Muster executable chosen above (on PATH in Muster terminals) and never have to manage PIDs, state files in /tmp, or raw WS URLs themselves.
...
| Goal                        | Command                                      | Notes |
|-----------------------------|----------------------------------------------|-------|
| List available / running   | `ORCA emulator list [--worktree <sel>]`     | Shows Muster-managed + raw serve-sim streams. Use output for explicit --device/--emulator. |
...
| Stop                       | `ORCA emulator kill [--device <id>]`        | Or let pane close / Muster quit clean up. |

Most support `--worktree <selector>` and explicit `--device <udid|name>` or `--emulator <id>` (from list) for targeting.

## Critical gotchas (teach agents)

- **Prefer `tap` over `gesture` for single taps** (same as raw serve-sim). Separate gesture begin/end can be interpreted as long-press due to WS overhead. The Muster wrapper uses the reliable quick sequence.
...
- Stale helpers / state are cleaned by Muster on quit, but agents should `kill` when done.
- Private APIs under the hood (SimulatorKit etc.) — version sensitive (Xcode updates can affect).

## Targeting devices & worktrees

- Default: current worktree's active emulator (resolved from shell cwd or Muster context).
...
- Muster-generated emulator id (for stability, like browserPageId): use `--emulator <id>` returned by list (recommended for scripts that persist ids).

`--worktree all` only for listing.

## Integration with the live pane (UI)

- Opening the emulator pane in Muster (or `attach`) makes that stream the "active" one for the worktree → CLI commands target it automatically.
...
Or let Muster quit / close the pane.

Orphans are cleaned by Muster (like agent-browser sessions).
...
Confirm `ORCA status --json` and `ORCA emulator list --json`, then drive the emulator while the live view is visible in Muster.

See also: orca-cli skill (terminals, worktrees, built-in browser), computer-use for desktop outside the simulator.

This skill is the Muster-native replacement for raw serve-sim when you want the visual + control integrated in the IDE.
